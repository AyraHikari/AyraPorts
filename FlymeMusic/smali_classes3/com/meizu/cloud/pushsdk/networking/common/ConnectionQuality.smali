.class public final enum Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;
.super Ljava/lang/Enum;
.source "SourceFile"


# annotations
.annotation system Ldalvik/annotation/Signature;
    value = {
        "Ljava/lang/Enum<",
        "Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;",
        ">;"
    }
.end annotation


# static fields
.field private static final synthetic $VALUES:[Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

.field public static final enum EXCELLENT:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

.field public static final enum GOOD:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

.field public static final enum MODERATE:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

.field public static final enum POOR:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

.field public static final enum UNKNOWN:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;


# direct methods
.method static constructor <clinit>()V
    .locals 11

    .line 27
    new-instance v0, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    const-string v1, "POOR"

    const/4 v2, 0x0

    invoke-direct {v0, v1, v2}, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;-><init>(Ljava/lang/String;I)V

    sput-object v0, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->POOR:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    .line 31
    new-instance v1, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    const-string v3, "MODERATE"

    const/4 v4, 0x1

    invoke-direct {v1, v3, v4}, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;-><init>(Ljava/lang/String;I)V

    sput-object v1, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->MODERATE:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    .line 35
    new-instance v3, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    const-string v5, "GOOD"

    const/4 v6, 0x2

    invoke-direct {v3, v5, v6}, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;-><init>(Ljava/lang/String;I)V

    sput-object v3, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->GOOD:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    .line 39
    new-instance v5, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    const-string v7, "EXCELLENT"

    const/4 v8, 0x3

    invoke-direct {v5, v7, v8}, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;-><init>(Ljava/lang/String;I)V

    sput-object v5, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->EXCELLENT:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    .line 44
    new-instance v7, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    const-string v9, "UNKNOWN"

    const/4 v10, 0x4

    invoke-direct {v7, v9, v10}, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;-><init>(Ljava/lang/String;I)V

    sput-object v7, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->UNKNOWN:Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    const/4 v9, 0x5

    new-array v9, v9, [Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    aput-object v0, v9, v2

    aput-object v1, v9, v4

    aput-object v3, v9, v6

    aput-object v5, v9, v8

    aput-object v7, v9, v10

    .line 23
    sput-object v9, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->$VALUES:[Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    return-void
.end method

.method private constructor <init>(Ljava/lang/String;I)V
    .locals 0
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "()V"
        }
    .end annotation

    .line 23
    invoke-direct {p0, p1, p2}, Ljava/lang/Enum;-><init>(Ljava/lang/String;I)V

    return-void
.end method

.method public static valueOf(Ljava/lang/String;)Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;
    .locals 1

    .line 23
    const-class v0, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    invoke-static {v0, p0}, Ljava/lang/Enum;->valueOf(Ljava/lang/Class;Ljava/lang/String;)Ljava/lang/Enum;

    move-result-object p0

    check-cast p0, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    return-object p0
.end method

.method public static values()[Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;
    .locals 1

    .line 23
    sget-object v0, Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->$VALUES:[Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    invoke-virtual {v0}, [Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;->clone()Ljava/lang/Object;

    move-result-object v0

    check-cast v0, [Lcom/meizu/cloud/pushsdk/networking/common/ConnectionQuality;

    return-object v0
.end method
