...
void main () {
    float origDepth = texture2D(depthTexture, v_TexCoordinate).r;
    float depth = texture2D(clippedDepthTexture, v_TexCoordinate).r;
    float revertDepth = 1.0 - depth;
    revertDepth = -revertDepth + focus;
    vec2 disCords = v_TexCoordinate + shift * revertDepth * scale; // D = d * depth *  s
    if (origDepth <= focus && texture2D(depthTexture, disCords).r >= focus) {
