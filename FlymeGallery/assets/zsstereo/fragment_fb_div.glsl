...
if (depth<= focus) {
    ...
    if ((depth - oridepth)>=0.1) {
     return 2.0;
    }
    else if ((oridepth - depth )>=0.1){
     return 1.0;
    }
...
else if (needblur(depthtexture,v_TexCoordinate+ shift * map,0.002,0.002,oridepth) ==2.0) {
   gl_FragColor = blur(texture, v_TexCoordinate+ shift * map);
}
else {
  gl_FragColor = blur(texture,v_TexCoordinate);
//  gl_FragColor = nearly(texture,v_TexCoordinate,v_TexCoordinate+ shift * map,0.2f);
//  gl_FragColor = texture2D(blurtexture, v_TexCoordinate);
//  gl_FragColor = blur13(texture, v_TexCoordinate,vec2(1.0, 1.0), vec2(0., 5.));
}
}
else {
  map = map * -1.0 + focus;
